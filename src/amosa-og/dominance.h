...
int is_dominated(double *func1,double *func2)
  {
    int i;
	...
	if(func1[i]<func2[i])
     ...
     if(((i_count_less+i_count_equal)==i_no_offunc) && i_count_less>0)
        return(TRUE);
     else 
        return(FALSE);
  }
   ...
   double find_unsign_dom(double *func1,double *func2)
     {
       int i;
       ...
       for(i=0;i<i_no_offunc;i++)
         {
            if((func1[i]-func2[i])!=0.0)
                 d_dominance=d_dominance*fabs(func1[i]-func2[i])/d_func_range[i];
...
/* The Following function calculates the signed amount of domination 
  ...
  double find_sign_dom(double *func1,double *func2)
     {
        int i;
        double d_dominance=1;
        for(i=0;i<i_no_offunc;i++)
          {
             if(fabs(func1[i]-func2[i])!=0)
                d_dominance=d_dominance*(func1[i]-func2[i])/d_func_range[i];
